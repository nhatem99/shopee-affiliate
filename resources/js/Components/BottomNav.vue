...
<template>
    <nav class="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-[--color-line] flex safe-area-inset-bottom">
        <Link
            v-for="item in items"
            :key="item.href"
            :href="item.href"
            class="flex-1 flex flex-col items-center justify-center py-3 gap-0.5 text-xs font-semibold transition"
            :class="current === item.href ? 'text-[--color-accent]' : 'text-[--color-muted]'"
        >
